...
const App = (props) => {
  ...
  const getSidebarWithTier = (categories_list, searched_client) => {
    ...
    const hasChildren = [];
    const items = categories_list.map((category, idx1) => {
      hasChildren.push(false);
      if(searched_client == "") {
        hasChildren[idx1] = true;
      }
      const client_available = RenderChilds(category.nome_categoria);
      return (
        <Item icon="fa-map-marker-alt" text={category.nome_categoria +" ("+ category.count_categoria +")"} key={idx1}>
          {client_available
          .filter( client => { if(client.name.includes(searched_client)) return client})
          .map( (el, idx2) => {
              if(idx2==0) {
                hasChildren[idx1] = true;
              }
              return <Item icon="fa-desktop" text={el.name} to={"/company"+el.company_name+"user"+el.id_client} key={idx2}/>
          })}
        </Item> 
      )
    })
    ...
    <div>
      ...
      :
        <>
          {toaster}
          {props.logged==true 
          ? <AdminLTE title={[<FontAwesomeIcon icon={["fas", "home"]} />, " Home"]} homeTo={"/"+props.id_company} titleShort={<FontAwesomeIcon icon={["fas", "home"]} />} theme="blue" sidebar={<><Item icon="fa-user-alt" key="-1" text="Account" to={"/"+props.id_company} /><Searchbar onChange={handleChange} includeButton="true" placeholder="Cerca..." />{getSidebarWithTier(props.categories_list, props.searched_client)}<a id="logoutButton" onClick={() => {props.TotalReset()}}><Item className="clickable" icon="fas-sign-out-alt" text={"Logout"} to={"/accedi"}></Item></a> </>}>
